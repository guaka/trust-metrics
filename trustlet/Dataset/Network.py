...
class WikiNetwork(WeightedNetwork):
    ...
    download: default true, try to download it from www.trustlet.org/datasets/svn
    ...
    def __init__(self, lang, date, current=False, bots=True, base_path = None,
                 dataset = None, force = False, download=True,
        ...
        # UPDATE
        if download:
            print "I'm updating c2 graph (from trustlet.org)"
            if not trustlet.helpers.svn_update(self.filepath+'.c2'):
                svnpath = trustlet.helpers.relative_path(self.filepath+'.c2','datasets')[0]
                if not trustlet.helpers.svn_update(svnpath):
                    print 'Abort update.'
